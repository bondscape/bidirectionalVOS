...
parser.add_argument("--sleap_path", help="path to sleap tracked.cleaned.h5 file", required=False, default="")
parser.add_argument("--csv_path", default="", help="explicit path to the annotations CSV (overrides the default {video}.annotations.csv)")
...
parser.add_argument('--plot-backend', type=str, default=None,
                    help="Matplotlib backend [for graphs]. (e.g. TkAgg, macosx, PyQt5, Qt5Agg). " \
...
dataframe_for_results = args.csv_path if args.csv_path else f"{exp}.annotations.csv"
...
def askCutieQuestions(thiszod, mp4_in, exp, direction, masks_dir_ds, masks_altdir_ds):
    ...
    while unanswered:
            ...
            # ADD after keyres = cv2.waitKey(INPUT_DELAY_MS):
            if keyres == -1 and not mpl_key_queue.empty():
                mpl_key = mpl_key_queue.get()
                key_map = {
                    'space': ord(' '), 'enter': 13, 'escape': 27,
                    's': ord('s'), 'g': ord('g'), 'z': ord('z'),
                    't': ord('t'), 'c': ord('c'), 'v': ord('v'),
                    '[': ord('['), ']': ord(']'), ',': ord(','),
                    '.': ord('.'), '-': ord('-'), '=': ord('='),
                    '+': ord('+'), '/': ord('/'), 'q': ord('q'),
                    "'": ord("'"), 'backspace': 127,
                }
                keyres = key_map.get(mpl_key, -1)
            if keyres != -1:
                print(f"Waitkey result: {keyres}")

            # drain clicks, too
            while not mpl_click_queue.empty():
                _, xdata = mpl_click_queue.get()
                update_boundaries(xdata)
    ...
    while True:
        ...
        if state == "CUTIE_R":
            ...
            if cmd == "CONTINUE":
                ...
                # done making determinations for this section
...
def mpl_worker_main(data_queue, click_queue, key_queue, plot_backend=None):
    # main worker handles sigint
    ...
    print(f"[mpl worker] using backend: {matplotlib.get_backend()}", flush=True)

    def on_click(event):
        if event.name != 'button_press_event' or event.button != 1 or \
               type(event.xdata) == type(None):
            return
        click_queue.put(("click", event.xdata))

    def on_key(event):
        key_queue.put(event.key)
    ...
    while True:
        if not data_queue.empty():
            ...
            if cmd == "draw":
                _, draw_kwargs = msg
                # unpack and redraw
                section_start = draw_kwargs["section_start"]
                section_end = draw_kwargs["section_end"]
                preview_start = draw_kwargs["preview_start"]
                preview_end = draw_kwargs["preview_end"]
                track_quality = draw_kwargs["track_quality"]
                iou = draw_kwargs["iou"]
                alt_iou = draw_kwargs["alt_iou"]
                zod_inventory = draw_kwargs["zod_inventory"]
                thiszod = draw_kwargs["thiszod"]
                mode = draw_kwargs["mode"]
                QUALITY_THRESHOLD = draw_kwargs["QUALITY_THRESHOLD"]
                control_window_mode = draw_kwargs["control_window_mode"]
                control_figure_section_selector = draw_kwargs["control_figure_section_selector"]
                worstLocs = draw_kwargs["worstLocs"]
                video_length = draw_kwargs["video_length"]

                if fig_local is None or not plt.fignum_exists(fig_local.number):
                    # show a couple of graphs to provide some context on tracking
                    fig_local = plt.figure(figsize=(6, 6))
                    fig_local.canvas.mpl_connect('button_press_event', on_click)
                    fig_local.canvas.mpl_connect('key_press_event', on_key)
                    ax_preview, ax1, ax2 = fig_local.subplots(nrows=3)

                    ax_preview.plot(np.arange(track_quality.shape[0]), 1-track_quality, color="g")
                    ax_preview.hlines(1-QUALITY_THRESHOLD, xmin=0, xmax=track_quality.shape[0])
                    ax_preview.set_title(f"ZOD-Quality Preview (#ZOD: {len(worstLocs)}. Threshold={1-QUALITY_THRESHOLD})")
                    ax_preview.set_ylim(0, 1.05)
                    ax_preview.set_xticks([])
                    plt.show(block=False)

                ax_preview, ax1, ax2 = fig_local.axes
                ax1.cla()
                ax2.cla()

                loc = section_start + np.argmax(track_quality[section_start:section_end+1])
                locquality = track_quality[loc]
                plt.suptitle(f"ZOD-{mode} [Local] Q={1-locquality:0.02} @ F:{loc} W:{section_start}-{section_end}")
...
def redraw_local_plot(thiszod, section_start, section_end, preview_start, preview_end):
    mpl_data_queue.put(("draw", {
        ...
        "iou": iou,
        "alt_iou": alt_iou,
        "zod_inventory": zod_inventory,
        "thiszod": thiszod,
        "mode": mode,
        "QUALITY_THRESHOLD": QUALITY_THRESHOLD,
        "control_window_mode": control_window_mode,
        "control_figure_section_selector": control_figure_section_selector,
...
if __name__ == '__main__':
    mpl_data_queue = mp.Queue()
    mpl_click_queue = mp.Queue()
    mpl_key_queue = mp.Queue()

    def do_quit(code):
        mpl_data_queue.put(None)
        mpl_proc.join(timeout=2)
    ...
    mpl_proc = mp.Process(target=mpl_worker_main,
                          args=(mpl_data_queue, mpl_click_queue, mpl_key_queue, args.plot_backend),
                          daemon=True)
    mpl_proc.start()
        ...
        for mode, locs in [["SWAPS", swapLocs], ["WORSTLOCS", worstLocs]]:
            for loc, q in locs:
                ...
                if not rescan_mode and r_end-r_start > 50:
                    ...
                    print(f"Trying to narrow {r_start}-{r_end}")
                    ...
                    r_end_orig = min(iou.shape[0]-1, loc + last_d)
                    print(f"start-end: {r_start_orig} {r_end_orig}")
